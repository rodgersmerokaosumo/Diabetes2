    ...
    [
        dcc.Upload(
            ...
            children=html.Button('Select Files', style={
                'background-color': '#007BFF', 'color': 'white', 'fontWeight': 'bold',
        ...
        html.Div(id='dataframe-info', style={'margin-top': '10px'}),
        html.Button('Show Missing Values Info', id='show-missing-values-btn', n_clicks=0,
                    style={'margin-top': '10px', 'width': '100%'}),
        html.Div(id='missing-values-info', style={'margin-top': '10px'}),
        ...
        html.Div(id='conversion-result', style={'margin-top': '10px'}),
        html.Button('Show Summary Statistics', id='show-summary-btn', n_clicks=0,
                    style={'margin-top': '10px', 'width': '100%'}),
        html.Div(id='summary-statistics', style={'margin-top': '10px'}),
        html.Button('Show Visualizations', id='show-visualizations-btn', n_clicks=0,
                    style={'margin-top': '10px', 'width': '100%'}),
        html.Div(id='visualization', style={'margin-top': '10px'}),
        html.Div(id='heatmap', style={'margin-top': '10px'}),
        html.Button('Resample', id='resample-button', n_clicks=0,
                    style={'margin-top': '10px', 'width': '100%'}),
        html.Div(id='resampled-data', style={'margin-top': '10px'}),
        html.Div(id='resampled-data-table', style={'margin-top': '10px'}),
        ...
        html.Button('Evaluate', id='evaluate-button', n_clicks=0,
                    style={'margin-top': '10px', 'width': '100%'}),
        html.Div(id='model-performance-metrics', style={'margin-top': '10px'}),
        html.Div(id='stored-df', style={'display': 'none'}),
        html.Button('Generate Report', id='generate-report-btn', n_clicks=0,
                    style={'margin-top': '10px', 'width': '100%'}),
        html.Button('Generate PDF Report', id='generate-pdf-report-btn', n_clicks=0,
                    style={'margin-top': '10px', 'width': '100%'}),
        html.Div(id='report-status', style={'margin-top': '10px'}),
        html.Div(id='pdf-report-status', style={'margin-top': '10px'}),
...
@app.callback(
    Output('pdf-report-status', 'children'),
    [Input('generate-pdf-report-btn', 'n_clicks')],
    [State('stored-df', 'children')]  # Assuming you store your DataFrame JSON in this hidden Div
)
def generate_pdf_report(n_clicks, stored_df_json):
    if n_clicks > 0 and stored_df_json:
        try:
            # Convert the stored DataFrame from JSON to DataFrame
            ...
            elements.append(Paragraph('Report Summary', styles['Heading1']))
            elements.append(Spacer(1, 12))

            # Example data table (convert DataFrame to a data table)
            ...
            # Generate the PDF
            doc.build(elements)
            return html.Div('PDF report generated successfully, check your server directory.')
        except Exception as e:
            return html.Div(f'Failed to generate PDF report: {str(e)}')
    return html.Div('Click the button to generate the PDF report.')
